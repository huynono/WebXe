...
const client = new OAuth2Client("13487730626-552o2asvklccucrjhggcjg8ppfp3d98j.apps.googleusercontent.com");
...
exports.googleLogin = async (req, res) => {
  ...
  try {
    const ticket = await client.verifyIdToken({
      idToken: token,
      audience: "13487730626-552o2asvklccucrjhggcjg8ppfp3d98j.apps.googleusercontent.com",
    });
    ...
    console.error(error);
    res.status(500).json({ message: "❌ Lỗi Google login", error: error.message });
  }
};
...
exports.facebookLogin = async (req, res) => {
  ...
  try {
    ...
    if (!fbData.email) {
      return res.status(400).json({ message: "Không thể lấy email từ Facebook" });
    }
    ...
    res.json({ message: "✅ Facebook login thành công", token: jwtToken, user });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "❌ Lỗi Facebook login", error: error.message });
  }
};
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    console.error('❌ Lỗi server khi xóa user:', error);
    res.status(500).json({ message: 'Lỗi server', error: error.message });
  }
};